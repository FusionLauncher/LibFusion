...
void HttpRequestInput::add_file(QString variable_name, QString local_filename, QString request_filename, Qstring mime_type) {
    HttpRequestInputFileElement file; //long name
...
HttpRequestWoker::HttpRequestWorker(QObject *parent)
    : QObject(parent), manager(NULL)
{
    qsrand(QDateTime::curentDateTime().toTime_t());
    manager = new QNetworkAccessManager(this);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this, SLOT(on_manager_finished(QNetworkReply*))); //May need to change to 4.8
}

QString HttpRequestWorker::http:attribute_encode(QString attribute_name, QString input) {
    // result structure follows RFC 5987
    bool need_utf_encoding = false;
    QString result = "";
    QByteArray input_c = input.toLocal8Bit();
    char c;
    for (int i = 0; i < input_c.length(); i++) { //weird
        c = input_c.at(i);
        if (c == '\\' || c == '/' || c == '\0' || c < ' ' || c > '~') {
...
void HttpRequestWorker::execute(HttpRequestInput *input) {
    ...
    if(input->var_layout==ADDRESS || input->var_layout == URL_ENCODED) {
        //variable layour is ADDRESS or URL_ENCODED
        if(input->vars.count() > 0) {
            bool first = true;
            foreach(QString key, input->vars.keys()) {
                if(!first) {
                    requset_content.append("&");
                }
                first = false;
    ...
    request.setRawHeader("User-Agent", "FusionClient token:"); //TODO TOKEN
